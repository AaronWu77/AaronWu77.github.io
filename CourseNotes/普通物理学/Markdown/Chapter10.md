...
在推到开始之前，我们先计算基本的一些物理量：
- 球相对于球心的转动惯量 $I_{c}=\frac{2}{5}mr^2$
- 初始状态：小球静止

推导：
- 设水平向左的总冲量为$I_{冲}=\int Fdt$，小球原本静止，获得向左的初速度 $v_0$, $\int F dt=mv_0$
- 计算冲量的力矩：$\tau=F\cdot \frac{4R}{5}$
- 角冲量：$J_{angular}=\int \tau dt = \int F\cdot \frac{4R}{5} dt = \frac{4R}{5}\int F dt = \frac{4R}{5}mv_0$
- 由于角冲量就等于角动量的变化，所以我们可以得到 $\frac{4R}{5}mv_0=I_c \omega_0$，所以 $\omega_0=\frac{2v_0}{r}$